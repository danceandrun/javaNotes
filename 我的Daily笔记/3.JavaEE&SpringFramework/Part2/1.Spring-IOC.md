...
获取依赖对象被反转了，它是被动获取；正转就是自己去new一个对象，自己获取对象。
...
依赖注入、事件、资源(resources加载配置文件)、i18n(internationalistion)国际化、校验、数据绑定、类型转换、Spring Expression Language、AOP
...
> 3. 有哪些子标签
>
> 在Spring中,要清楚`<bean>`标签，以及子标签`<property>`
